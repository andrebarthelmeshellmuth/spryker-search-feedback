...
namespace SprykerCommunityTest\Zed\SearchFeedback\Communication\Acl;
...
class BackOfficeAccessAnalyzerTest extends Unit
{
     ...
     * @var string
     */
    protected const MODULE_NAME = 'search-feedback-gui';
    ...
    /**
     * Exactly the split this package's own README suggests: let a role view and reply, but deny it the
     * status-change action. That role still very much reaches the module, and must not be counted as
     * having no access.
     */
    public function testASingleActionDenyDoesNotCancelOutModuleAccess(): void
    {
        ...
        $analyzer = $this->createAnalyzer([
            2 => [
                ...
                [static::MODULE_NAME, 'detail', 'change-status', 'deny'],
            ],
        ]);
    ...
    protected function createAnalyzer(array $rulesByRoleId, ?array $roleIdsByGroupId = null): BackOfficeAccessAnalyzer
    {
        ...
        $aclFacadeMock = $this->createMock(SearchFeedbackToAclFacadeInterface::class);
